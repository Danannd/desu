...
public class DataHelper {
    ...
    private static List<ColorWrapper> sColorWrapper = new ArrayList<>();
                ...
                if(!(constrain == null || constrain.length() == 0)) {
                    for(ColorSuggestion suggestion : sColorSuggestions) {
                        if(suggestion.getBody().toUpperCase()
                                .startsWith(constrain.toString().toUpperCase())) {
    ...
    public static void findColors(Context context, String query, final OnFindColorsListener listener) {
        ...
        new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constrain) {
                ...
                if(!(constrain == null) || constrain.length() == 0) {
                    for(ColorWrapper color : sColorWrapper) {
    ...
    private static void initColorWrapperList(Context context) {
        ...
        if(sColorWrapper.isEmpty()) {
            String jsonString = loadJson(context);
            sColorWrapper = deserializeColors(jsonString);
        }
    }
